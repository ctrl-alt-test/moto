...
const char *preprocessed_scene_frag =
 ...
 "{"
   ...
   "return v;"
 "}"
 "material d(float v,vec3 m,vec3 M,float f)"
 "{"
 ...
 "{"
   ...
   "{"
     ...
     "float c=t(r,x,x+vec3(-.08,-.07,.3),f);"
     "a=min(a,c-mix(.035,.02,smoothstep(.25,.4,f)));"
     "r=r-x-vec3(0,.1,.15);"
     "r.xz*=s(.2);"
     "r.xy*=s(-.2);"
     "c=r.x-.02;"
     "r.xz*=s(.25);"
     "c=-min(c,-n(r,vec3(.04,.05,.08)));"
     "a=min(a,c);"
 ...
 "{"
   ...
   "return v>=MOTO_ID&&v<=MOTO_MOTOR_ID?"
     "m=e(m,true,M),f=e(f,false,M),d(v,m,f,M):"
     "v==DRIVER_ID?"
 ...
 "{"
   "if(f>0.)"
     "{"
       ...
       "return normalize(m.xyy*h(v+m.xyy*.01).x+m.yyx*h(v+m.yyx*.01).x+m.yxy*h(v+m.yxy*.01).x+m.xxx*h(v+m.xxx*.01).x);"
     "}"
   "{"
 ...
 "{"
   ...
   "y*=y*y*y*y;"
   "return vec3(0)+r.emissive+(E*(i.y*.5+.5)+b*clamp(dot(i,M),0.,1.)*v)*r.albedo;"
 "}"
 "void u(vec2 v,out vec3 f,out vec3 i)"
 "{"
   "vec3 m=d(vec3(-2,.5,-.2),true,M),y=normalize(m+vec3(10,0,0)-m),r=d(vec3(0,1,0),false,M);"
   "if(abs(dot(y,r))>.99)"
     "r=vec3(1,0,0);"
   "vec3 z=normalize(cross(y,r));"
   "r=normalize(cross(z,y));"
   "f=m;"
   "i=normalize(y+v.x*z+v.y*r);"
 "}"
 ...
 "{"
   "n();"
   "vec2 y=(m/f.xy*2.-1.)*vec2(1,f.y/f.x);"
   "float r=M;"
   "r+=n(vec3(m,.001*M))*.008;"
   "vec2 x=y*2.-1.;"
   "vec3 s=a,D=Z(s,i)*normalize(vec3(x,z-length(x)*c));"
   "u(y,s,D);"
   "vec3 e;"
...
const char *fxaa_frag =
 ...
 "void main()"
 "{"
   "vec2 m=1./T,f=gl_FragCoord.xy*m;"
   "vec4 i=vec4(f,f-m*.5);"
   "vec3 M=vec3(.299,.587,.114);"
   "float r=dot(textureLod(k,i.zw,0.).xyz,M),y=dot(textureLod(k,i.zw+vec2(1,0)*m.xy,0.).xyz,M),z=dot(textureLod(k,i.zw+vec2(0,1)*m.xy,0.).xyz,M),x=dot(textureLod(k,i.zw+vec2(1)*m.xy,0.).xyz,M),a=dot(textureLod(k,i.xy,0.).xyz,M),D=min(a,min(min(r,y),min(z,x)));"
   "a=max(a,max(max(r,y),max(z,x)));"
   "f=vec2(-r-y+z+x,r+z-y-x);"
   "r=1./(min(abs(f.x),abs(f.y))+1./128.);"
   "f=min(vec2(8),max(vec2(-8),f*r))*m.xy;"
   "vec3 s=.5*(textureLod(k,i.xy+f*(1./3.-.5),0.).xyz+textureLod(k,i.xy+f*(2./3.-.5),0.).xyz),c=s*.5+.25*(textureLod(k,i.xy+f*-.5,0.).xyz+textureLod(k,i.xy+f*.5,0.).xyz);"
   "r=dot(c,M);"
   "v=r<D||r>a?"
     "vec4(s,1):"
     "vec4(c,1);"
